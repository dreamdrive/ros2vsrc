...
// PCからUSBシリアルでも使えますが、適宜デバイス名(#define SERIAL_PORT)を変更するのと、VS-RC003のUARTに合わせてレベル変換してください。
//
// vsrc_connectからの変更点としては、VSRCのコマンドをそのままSubscribするのではなく、カスタムメッセージVsrcControlをSubscribします。
// VsrcControlの内容はmsgファイルを参照してください。
// また、VS-RC003の状態として、VsrcStateとBatteryStateをPublishします。電圧監視やモーションの再生状況を監視出来ます。
// 以上、vsrc_connectとはまったく互換性がありません。
//
// ※本ノードとrqt_ez_publisherだけでも動作テスト出来ますが、念のためvsrc_connect2用のvsrc_control2も作りました。
//
//
// MIT License
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "vsrc_connect2");
	ros::NodeHandle n;
	...
	//Subscriber
	ros::Subscriber vsrc_sub = n.subscribe("vsrc_control2", 10, vsrc_ctl_callback);

	char device_name[]=SERIAL_PORT;	// デバイス名セット
	fd1=open_serial(device_name);	// シリアルオープン
